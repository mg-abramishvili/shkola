...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.schedulesimple.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.schedulesimples.update", [$schedulesimple->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="schsm_title">{{ trans('cruds.schedulesimple.fields.schsm_title') }}</label>
                <input class="form-control {{ $errors->has('schsm_title') ? 'is-invalid' : '' }}" type="text" name="schsm_title" id="schsm_title" value="{{ old('schsm_title', $schedulesimple->schsm_title) }}" required>
